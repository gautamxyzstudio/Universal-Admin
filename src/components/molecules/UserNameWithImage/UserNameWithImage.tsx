...
const UserNameWithImage: React.FC<IUserNameWithImageProps> = ({
    ...
    return (
        <div className="flex w-full gap-x-[10px] h-auto flex-row items-center justify-start">
            <UserImageRenderer imageStyle={imageStyle + ' w-10 h-10'} image={image} name={name} type={type} />
            <div className={divStyle + ' flex flex-col gap-y-1'}>
                {name && <span className={nameStyle + ' text-[16px] leading-[20px] text-Black'}>{name}</span>}
                {companyName && (
                ...
                {joinDate && <span className="text-[14px] leading-[18px] text-disable">Joined - {joinDate}</span>}
            </div>
        </div>
    );
};
