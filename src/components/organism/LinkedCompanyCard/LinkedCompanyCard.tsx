...
const LinkedCompanyCard: React.FC<ILinkedCompanyCardProps> = ({
    ...
    return (
        <div className="w-full px-6 relative py-4 bg-statusLightGreen">
            ...
            />
            <div className="h-2" />
            {companyEmail && <IconWithText text={companyEmail} icon={Icons.emailIcon} />}
            {companyLocation && <IconWithText text={companyLocation} icon={Icons.locationPin} />}
            <div onClick={onPressCross} className="absolute cursor-pointer right-2 top-2">
