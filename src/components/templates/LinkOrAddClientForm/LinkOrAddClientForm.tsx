...
const LinkOrAddClientFrom: React.FC<ILinkOrAddClientFrom> = ({
  ...
  const validateForm = () => {
    ...
    if (type === 'add') {
      ...
      if (!password) {
        isValid = false;
        setState({ ...state, passwordError: STRINGS.required_field });
      }
      if (!location) {
        isValid = false;
        setState({ ...state, locationError: STRINGS.required_field });
      }
      ...
      if (!name) {
        isValid = false;
        setState({ ...state, clientNameError: STRINGS.required_field });
      }
        ...
        onPressAddEmployee &&
          onPressAddEmployee({
            ...
            client: {
              ...
              industry: industry,
              location: location,
              status: state.status ?? IClientStatus.ACTIVE,
  ...
  return (
    ...
    >
      <div className="px-6 flex h-[88%] overflow-scroll flex-col gap-y-4 pt-6 scrollbar-none">
        ...
        {type === 'add' && (
          <PasswordInput
            ...
            onChange={(e) =>
              onChangeTextField(e.target.value, CompanyStateFields.password)
            }
          />
        )}
        {type === 'add' && (
          <div className="flex flex-col">
            {STRINGS.status}
            <Switch
              checked={state.status === IClientStatus.ACTIVE}
              ...
              label={
                state.status === IClientStatus.ACTIVE ? 'Active' : 'InActive'
              }
              className={
      ...
      <div className="bg-white z-10 w-[436px] fixed bottom-0 px-6 pt-4 pb-6">
        <CustomButton
          fullWidth
