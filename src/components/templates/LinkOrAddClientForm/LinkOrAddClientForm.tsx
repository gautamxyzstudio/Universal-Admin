...
import { ILinkClientFrom, ILinkOrAddClientFrom } from './LinkOrAddClientForm.types';
...
const LinkOrAddClientFrom: React.FC<ILinkOrAddClientFrom> = ({
    ...
    const [linkedCompany, setLinkedCompany] = useState<null | ICompany>(null);
    ...
    useEffect(() => {
        if (selectedCompany) {
            setLinkedCompany(selectedCompany);
        }
    }, [selectedCompany]);

    const handleClickOutside = (event: object, reason: 'backdropClick' | 'escapeKeyDown') => {
        ...
        >
            <div className="px-6 flex flex-1 flex-col gap-y-6 pt-6">
                <FormTextInput
                    ...
                    size="medium"
                    onChange={(e) => onChangeTextField(e.target.value, CompanyStateFields.clientName)}
                ...
                <FormTextInput
                    ...
                    size="medium"
                    onChange={(e) => onChangeTextField(e.target.value, CompanyStateFields.companyName)}
                ...
                <FormTextInput
                    ...
                    size="medium"
                    onChange={(e) => onChangeTextField(e.target.value, CompanyStateFields.industry)}
                ...
                <FormTextInput
                    value={state.email}
                    size="medium"
                    onChange={(e) => onChangeTextField(e.target.value, CompanyStateFields.email)}
                ...
                <FormTextInput
                    ...
                    size="medium"
                    onChange={(e) => onChangeTextField(e.target.value, CompanyStateFields.contactNumber)}
                ...
                <FormTextInput
                    ...
                    size="medium"
                    onChange={(e) => onChangeTextField(e.target.value, CompanyStateFields.location)}
                ...
                {linkedCompany ? (
                    <>
                        <div
                            className="
                    items-center gap-x-1 flex flex-row"
                        >
                            <svg
                        ...
                        <LinkedCompanyCard
                            ...
                            onPressCross={onPressCrossHandler}
                        />
                    </>
                ) : (
                    <div onClick={onPressLink} className="cursor-pointer items-center gap-x-1 flex flex-row">
            ...
            <div className="bg-white px-6 pt-4 pb-6">
                <CustomButton
                    fullWidth
                    disabled={linkedCompany === null}
                    title={STRINGS.confirm}
                    buttonType={'primary-small'}
                    variant={'contained'}
                    onClick={() => onPressAddEmployee({ client: selectedClient, company: linkedCompany })}
                />
            </div>
        </FormDrawer>
    );
