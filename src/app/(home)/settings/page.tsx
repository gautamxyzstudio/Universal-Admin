...
const settings = () => {
  ...
  const [setectedTab, setSetectedTab] = useState<React.ReactNode>(<FaqTab/>);
  ...
  const handleFaqTab = () => {
    setSetectedTab(<FaqTab />);
  };
  const handleMyAccountTab = () => {
    setSetectedTab("My Account selected tab");
  };
  const handleNotificationsTab = () => {
    setSetectedTab("Notifications selected tab");
  };
  return (
    <div className="w-full h-full flex">
      ...
      <div className="w-[72%] h-full">{setectedTab}</div>
    </div>
  );
};
